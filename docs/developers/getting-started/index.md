...
- **Fast Transactions**: Users enjoy 50ms pre-confirmations, enhancing the overall experience.
- **Value Flow Alignment**: Sequencing fees generated by rollups are distributed between Ethereum validators and rollup operators. Rollup owners can configure their revenue share through [Puffer Preconfs](https://docs.puffer.fi/puffer-preconf/puffer-preconf-intro) (Puffer UniFi AVS), giving them flexibility over how transaction processing income is allocated.
- **Fast Withdrawals from L2 to L1 (Ethereum)**: Users can withdraw funds from L2 to L1 in minutes instead of the standard 7-day dispute period. This is achieved through **TEE (Trusted Execution Environment) proofs** that provide cryptographic guarantees of execution correctness.
