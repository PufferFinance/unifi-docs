...
| 41     | COINBASE  | block.coinbase      | Always returns the `SequencerFeeVault` predeploy `0x4200000000000000000000000000000000000011`. It is a constant and cannot be used to identify the gateway that built the block. | Gets the block's beneficiary address |
...
| 4A     | BLOBBASEFEE | block.blobbasefee    | Returns 1 | Returns the value of the blob base-fee of the current block |
